...
```{r}
draw_ggraph(g, normDownWeights, labels)
```
...
```{r, results = 'hide'}
draw_networkD3(g, normDownWeights, labels)
```
...
```{r, results = 'hide'}
draw_visNetwork(g, normDownWeights, labels)
```
```{r, echo=FALSE, out.width = "100%", out.height="100%"}
...
```{r}
cat( to_dot(g, normDownWeights, labels) )
```
